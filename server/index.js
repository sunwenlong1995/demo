...
const app = express()
const DATA_FILE = path.join(path.dirname(new URL(import.meta.url).pathname).replace(/^\/([A-Za-z]):\//, '$1:/'), '../data.json')
...
app.use(express.static(path.join(path.dirname(new URL(import.meta.url).pathname).replace(/^\/([A-Za-z]):\//, '$1:/'), '../dist')))
...
app.get('*', (req, res) => {
  res.sendFile(path.join(path.dirname(new URL(import.meta.url).pathname).replace(/^\/([A-Za-z]):\//, '$1:/'), '../dist/index.html'))
})
